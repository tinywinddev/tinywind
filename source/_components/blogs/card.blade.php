<div class="flex flex-col max-w-sm overflow-hidden border border-b-2 border-gray-100 border-gray-200">
    <div class="flex-shrink-0">
        <img
            src="https://i.picsum.photos/id/925/536/354.jpg?hmac=LOwNDNY01Usnn_7f-5NwaVkOcFMKNLAIF8Fym7uTiAg"
            alt="Image alt text"
            class="object-cover w-full h-48"
        >
    </div>

    <div class="flex-1">
        <div class="p-6 space-y-2">
            <a
                href="#"
                class="text-xl font-medium text-blue-500 hover:text-blue-600 focus:text-blue-600 hover:underline focus:underline"
            >Introduction to TailwindCSS components and more</a>
            <p class="leading-relaxed text-gray-500 hover:text-gray-600">
                Lorem ipsum dolor sit amet, consectetur adipiscing elit. Nunc
                porta dolor ut arcu aliquam consectetur. Nullam interdum enim eu elit convallis tincidunt. In sit amet
                arcu ultrices, mattis metus et, aliquam nisl.
            </p>
        </div>
        <div class="flex items-center px-6 py-4 space-x-4 bg-gray-50">
            <img
                alt=""
                src="https://thispersondoesnotexist.com/image"
                class="object-cover w-16 h-16 rounded-full "
            />
            <div class="space-y-1">
                <p class="font-medium">Ryan Chandler</p>
                <p class="text-sm text-gray-600 uppercase">May 27, 2021</p>
            </div>
        </div>
    </div>
</div>
